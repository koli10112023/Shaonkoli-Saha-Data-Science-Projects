...
def request_campaign(username,id):
    if request.method == 'POST':
        ...
        negotiation = request.form['negotiation']
        link_data.message = message
        link_data.negotiation = negotiation
...
def accept_request(username,id,camp_id):
    if request.method == 'POST':
        ...
        campaign_data = campaign.query.filter(campaign.id==camp_id).first()
        print(campaign_data)
        campaign_data.budget = link_data.negotiation
        db.session.commit()
